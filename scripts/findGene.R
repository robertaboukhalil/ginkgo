...
setwd(main_dir)
binList=read.table(paste("bins_", bm, sep=""), header=FALSE, sep="\t", as.is=TRUE)
...
orig=read.table("SegRaw", header=TRUE, sep="\t", as.is=TRUE)

l=dim(orig)[1] #Number of bins
w=dim(orig)[2] #Number of samples
lab=colnames(orig) #Sample labels

#Find bin corresponding to requested gene
chr=geneList[[1]][which(geneList[[4]]==gene)]
loc=geneList[[2]][which(geneList[[4]]==gene)]
u=subset(binList, binList[[1]]==chr)[2]<loc
bin=as.integer(rownames(u)[max(which(u))])
...
for(k in 1:w){
  ...
  jpeg(filename=paste(lab[k], ".jpeg", sep=""), width=2000, height=1400)
    ...
    abline(v=bin, h=normal[,k][bin], col='red')
