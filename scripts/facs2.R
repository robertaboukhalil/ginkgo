...
#Find bin corresponding to requested genes
for (i in 1:cnt) {
...
for(k in 1:w){
  ...
  jpeg(filename=paste(lab[k], ".jpeg", sep=""), width=2000, height=1400)
    ...
    layout(matrix(c(1, 2, 3, 3, 4, 4, 5, 5, 6, 6, 6, 6), 3, 4, byrow=TRUE))

    #plot stats table
    textplot(stats, halign="center", valign="top", mar=c(0,0,3,0), show.rownames=FALSE, show.colnames=FALSE)
    ...
    if (cnt <= 3) {
      for (i in 1:cnt) {
        abline(v=bin[i], h=final[,k][bin[i]], lwd=3, col=colors[i])
      }
    } else {
      for (i in 1:cnt) {
        points(bin[i], final[,k][bin[i]], pch=13, cex=4, lwd=2, col=colors[i])
      }
    }
